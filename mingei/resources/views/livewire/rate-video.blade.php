<div class="flex items-center mt-8">
    <button wire:click="rateGoodVideo" class="{{ $isActiveGood ? 'bg-red-500 text-white' : '' }} mr-6 flex items-center outline-none ring-2 ring-gray-300 font-medium rounded-full text-xs px-5 py-2.5 text-center">
        ...
        おもしろいね ({{ formatNumber($goodCount) }})
    </button>
    <button wire:click="rateBadVideo" class="{{ $isActiveBad ? 'bg-blue-500 text-white' : '' }} flex items-center outline-none ring-2 ring-gray-300 font-medium rounded-full text-xs px-5 py-2.5 text-center">
