<!-- <nav x-data="{ mobileMenuOpen: false, mobileSearchOpen: false, scrolled: false, prevScroll: 0 }" x-init="() => { window.addEventListener('scroll', () => { scrolled = window.pageYOffset > 64; prevScroll = window.pageYOffset; }) }" :class="{'-translate-y-16': scrolled && window.pageYOffset > 64 }" @scroll.window="prevScroll = window.pageYOffset" class="fixed top-0 left-0 z-10 w-full bg-orange-400 transition-transform duration-300 ease-in-out"> -->
<nav x-data="{ mobileMenuOpen: false, mobileSearchOpen: false, mobileMenuHidden: false, prevScroll: 0 }" x-init="() => { window.addEventListener('scroll', () => { mobileMenuHidden = window.pageYOffset > 64; mobileMenuHidden = prevScroll < window.pageYOffset; prevScroll = window.pageYOffset;  }) }" :class="{'-translate-y-16': mobileMenuHidden, 'translate-y-0': !mobileMenuHidden }" @scroll.window="prevScroll = window.pageYOffset" class="fixed top-0 left-0 z-10 w-full bg-orange-400 transition-transform duration-300 ease-in-out">
    <!-- <nav x-data="{ mobileMenuOpen: false, mobileSearchOpen: false, scrolled: false, prevScroll: 0 }" x-init="() => { window.addEventListener('scroll', () => { scrolled = window.pageYOffset > 64; }) }" :class="{'-translate-y-16': scrolled && window.pageYOffset > prevScroll, 'translate-y-0': !scrolled || window.pageYOffset <= 64 }" @scroll.window="prevScroll = window.pageYOffset" class="fixed top-0 left-0 z-10 w-full bg-orange-400 transition-transform duration-300 ease-in-out"> -->

    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex py-3" x-transition.scale.10 x-show="mobileSearchOpen" @click.away="mobileSearchOpen = false">
            <button @click="mobileSearchOpen = false" type="button" class="text-white grow-0 px-2" style="white-space: nowrap; padding: 5px; margin-right: 10px;">
                閉じる
            </button>
            <div class="w-full">
        ...
        <div x-show="!mobileSearchOpen" class="flex h-16 items-center justify-between">
            ...
            <div class="hidden lg:block">
                <div class="ml-4 flex items-center md:ml-6">
                    <a href="{{ route('notifications.index') }}" type="button" class="relative rounded-full bg-orange-400 p-1 text-white hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                        <span class="sr-only">View notifications</span>
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                        </svg>
                        @if (GetUnreadNotificationCount() !== 0)
                        <div class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900">{{ GetUnreadNotificationCount() }}</div>
                        @endif
                    </a>
